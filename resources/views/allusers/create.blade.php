...
<div class="rounded-lg border-4 border-green-500 my-2 py-2 px-2">
    <form action="{{route('allusers.store')}}" method="POST" class="mt-5" enctype="multipart/form-data">
        @csrf
        <select hidden name="role_id" id="" class="w-full rounded-lg border-2 border-red-300 my-2">
        ...
        <input type="file" name="image" accept="image/*" class="w-full border-2 border-gray-400 my-2">
        </div>
        @error('image')
            <p class="text-red-400 text-xs -mt-2">{{$message}}</p>
        @enderror
